var store = [{
        ...
        "excerpt":"您可於此 design_pattern repo 下載 Design Pattern 系列程式碼。 前言 這次的Pattern讓我想到以前做過的一個App，但當時還沒有學習到Pattern，所以沒有用 Pattern 來處理，現在發現這個功能很適合套用 prototype pattern 這是一個用來編輯音樂燈光秀的App，有興趣的讀者可以下載玩玩看🙂 Asante TapTap 3 需求 今天收到了客人的需求，客人反應編輯完一條燈光還要編輯另外六條好浪費時間，能不能新增 Copy &amp; Paste 的功能，加快編輯以節省時間，如下圖 物件導向分析 (OOA) 理解需求後，讓我們來快速實作物件導向分析吧! 當我們需要複製 LightShowData 時，只需要同樣的 jsonObject 資料重新 new 一個 LightShowData 即可複製一份 察覺 Forces 來看看上面這樣的設計會有哪些問題 如果我們的 constructor 很複雜，參數非常多，那麼重新 new 一個實體會需要知道很多細節。 如果 constructor 創建實體的過程，是很耗時複雜的計算，重新 new 一個實體會讓創建過程效率變差。 套用 Prototype...","categories": ["Design Pattern"],
